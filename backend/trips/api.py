...
def list_trips(request):
    ...
    return [TripSchema(id=t.id, name=t.name, steps=[StepSchema(id=s.id, name=s.name) for s in t.steps.all()]) for t in trips]

@api.post("/trips", response=TripSchema)
def create_trip(request, data: TripSchema):
    trip = Trip.objects.create(name=data.name)
    return TripSchema(id=trip.id, name=trip.name, steps=[])
...
def add_step(request, trip_id: int, data: StepSchema):
    ...
    return StepSchema(id=step.id, name=step.name)
